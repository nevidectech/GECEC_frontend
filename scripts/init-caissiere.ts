...
async function initCaissiere() {
    const email = process.env.CAISSIERE_EMAIL || "caissiere@gecec.cd"
    const password = process.env.CAISSIERE_PASSWORD || "caissiere1234"

    console.log(`Initialisation de l'utilisateur caissiere ${email}...`)
    ...
    if (existingUser) {
        ...
        const { data: userData, error: authError } = await supabase.auth.admin.createUser({
            ...
            password,
            email_confirm: true,
            user_metadata: { full_name: "Caissiere", username: "caissiere" },
            app_metadata: { function: "caissiere" },
        })
