...
async function initSuperviseur() {
    const email = process.env.SUPERVISEUR_EMAIL || "superviseur@gecec.cd"
    const password = process.env.SUPERVISEUR_PASSWORD || "superviseur1234"

    console.log(`Initialisation de l'utilisateur superviseur ${email}...`)
    ...
    if (existingUser) {
        ...
        const { data: userData, error: authError } = await supabase.auth.admin.createUser({
            ...
            password,
            email_confirm: true,
            user_metadata: { full_name: "Superviseur", username: "superviseur" },
            app_metadata: { function: "superviseur" },
        })
